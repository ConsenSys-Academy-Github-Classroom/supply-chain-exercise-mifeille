...
contract SupplyChain {
  ...
  function buyItem(uint sku) payable paidEnough(msg.value) forSale(sku) checkValue(sku) public {
  ...
  function shipItem(uint sku) sold(sku) verifyCaller(msg.sender) public {
    if (msg.sender != items[sku].seller) {
  ...
  // 3. Call the event associated with this function!
  function receiveItem(uint sku) shipped(sku) verifyCaller(msg.sender) public {
    if (msg.sender != items[sku].buyer) {
